...
def sample_image_mnist(n_row, batches_done, in_distribution_index, out_distribution_index, real_dataset_in, real_dataset_out, index_in_distribution, index_out_distribution, generator, sample_size):
    ...
    # Get labels ranging from 0 to n_classes for n rows
    condition = np.array([num for _ in range(n_row) for num in np.linspace(cfg_data.min_dataset, cfg_data.max_dataset, 10, endpoint=True)])
    gen_imgs = generate_sample_from_z_condition(generator, z, condition)(z, labels)
...
def get_main_data():
    ...
    return dataset, dataloader, testset, index_in_distribution, index_out_distribution, real_dataset_in, real_dataset_out, in_distribution_index, out_distribution_index

def compute_and_store_results(df_acc_gen, batches_done, in_distribution_index, out_distribution_index, real_dataset_in, real_dataset_out, index_in_distribution, index_out_distribution, generator):
    if cfg.experiment == 'rotation_dataset':
        ...
        df = pd.DataFrame([np.mean(se_gan[in_distribution_index])], columns=['mse_in'])
        df['mse_out'] = np.mean(se_gan[out_distribution_index])
        df['fid_in'] = fid_in[0]
        df['fid_out'] = fid_out[0]
        df['kid_in'] = kid_in[0]
        df['kid_out'] = kid_out[0]
...
def train_gan_model():
    ...
    for epoch in range(epoch_start, cfgan.n_epochs):
        ...
        if epoch == 0:
            ...
            df_acc_gen = compute_and_store_results(df_acc_gen, epoch, in_distribution_index, out_distribution_index, real_dataset_in, real_dataset_out, index_in_distribution, index_out_distribution, generator)
